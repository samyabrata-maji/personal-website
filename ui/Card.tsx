...
export default function Card({ title, imgHref, children }: CardPrpos) {
  return (
  ...
  return (
    <div className={`${className}`}>
      ...
      <div className="flex w-[100%] items-center justify-center">
        <Link
          href={"/blogs"}
          className="text-[#ADD8e6] underline underline-offset-4"
        >
          Show more
        </Link>
      </div>
...
export function CardBlogContainer({
  ...
  return (
    <div className={`${className}`}>
      <h1 className="px-6 text-4xl font-black">{title}</h1>
      <div
        className={`card-container grid w-[100%] grid-cols-2 gap-8 px-6 py-6 max-md:grid-cols-3 max-sm:grid-cols-1`}
      >
        {children}
      </div>
      <div className="flex w-[100%] items-center justify-center">
